...
class UserStore extends GetxService {
  ...
  final RxString _traineeEmailAddress = ''.obs;

  String get traineeEmailAddress => _traineeEmailAddress.value;
  ...
  void _loadUserFromStorage() {
    try {
      ...
      final storedToken = _storageService.getString(
        StorageKeys.STORAGE_USER_TOKEN_KEY,
      );
      ...
      // Load the trainee email address
      final storedEmail = _storageService.getString(
        StorageKeys.STORAGE_USER_EMAIL_KEY,
      );
      if (storedEmail.isNotEmpty) {
        _traineeEmailAddress.value = storedEmail;
        logger.i(
          "UserStore: Loaded trainee email - ${_traineeEmailAddress.value}",
        );
      } else {
        _traineeEmailAddress.value = '';
        logger.i("UserStore: No trainee email found in storage.");
      }
    } catch (e) {
  ...
  Future<void> saveTraineeEmailAddress(String email) async {
    await _storageService.setString(StorageKeys.STORAGE_USER_EMAIL_KEY, email);
    _traineeEmailAddress.value = email;
    logger.i("UserStore: Saved trainee email address.");
  }
  ...
  void _initializeSubscriptionState() {
    if (_profile.value?.traineeProfile != null) {
      ...
      logger.i(
        "UserStore: Initialized subscription state - isPremium: ${_isPremium.value}, remainingMessages: ${_remainingFreeMessages.value}",
      );
    } else {
  ...
  Future<void> decrementFreeMessages() async {
    if (!_isPremium.value && _remainingFreeMessages.value > 0) {
      ...
      logger.i(
        "UserStore: Decremented free messages to ${_remainingFreeMessages.value}",
      );
    }
  }
  ...
  Future<void> onLogout() async {
    ...
    await _storageService.remove(StorageKeys.STORAGE_USER_EMAIL_KEY);
    _token.value = '';
    _profile.value = null;
    _isLogin.value = false;
    _traineeEmailAddress.value = '';
